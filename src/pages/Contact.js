...
const ContactPage = () => {
    ...
    return (
        <div className={styles.container}>
            ...
            <form onSubmit={submitHandler} className={`w-50 mx-auto border rounded p-4 my-5 shadow ${styles.form}`}>
                <div className="my-3">
                    <label className="form-label" htmlFor="name">Name</label>
                    <input type="text" className="form-control bg-light" id="name" placeholder="Enter your name" value={name} onChange={event => { inputChangeHandler("name", event.target.value) }} required />
                    {nameError && <p className="text-danger">Please enter your name</p>}
                </div>
                <div className="my-3">
                    <label className="form-label" htmlFor="email">Email</label>
                    <input type="email" className="form-control bg-light" id="email" placeholder="Enter your email" value={email} onChange={event => { inputChangeHandler("email", event.target.value) }} required />
                    {emailError && <p className="text-danger">Please enter a valid email</p>}
                </div>
                <div className="my-3">
                    <label className="form-label" htmlFor="message">Message</label>
                    <textarea className="form-control bg-light" placeholder="Enter your message" rows="5" value={message} onChange={event => { inputChangeHandler("message", event.target.value) }} required />
